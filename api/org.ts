...
export const orgApi = {
  ...
  getUserPermissions: async (): Promise<UserPermissionInfo> => {
    const response = await fetch(`${API_BASE}/api/org/user-permissions`, { headers: getHeaders() });
    return handleResponse(response);
  },

  // 获取用户部门相关项目列表（从project服务获取）
  listUserDepartmentProjects: async (): Promise<{ total: number; projects: Project[] }> => {
    const response = await fetch(`${API_BASE}/api/org/user-department-projects`, { headers: getHeaders() });
  ...
  listDepartments: async (): Promise<Department[]> => {
    const response = await fetch(`${API_BASE}/api/org/departments`, { headers: getHeaders() });
    return handleResponse(response);
  },

  createDepartment: async (payload: { name: string; description?: string; parent_id?: number }): Promise<Department> => {
    const response = await fetch(`${API_BASE}/api/org/departments`, {
      method: 'POST',
      headers: getHeaders(),
  ...
  getDepartment: async (departmentId: number): Promise<Department> => {
    const response = await fetch(`${API_BASE}/api/org/departments/${departmentId}`, { headers: getHeaders() });
    return handleResponse(response);
  },

  updateDepartment: async (departmentId: number, payload: { name?: string; description?: string; parent_id?: number }): Promise<Department> => {
    const response = await fetch(`${API_BASE}/api/org/departments/${departmentId}`, {
      method: 'PUT',
      headers: getHeaders(),
      body: JSON.stringify(payload),
    });
  ...
  getDepartmentMembers: async (departmentId: number): Promise<DepartmentMember[]> => {
    const response = await fetch(`${API_BASE}/api/org/departments/${departmentId}/members`, { headers: getHeaders() });
    return handleResponse(response);
  },

  addDepartmentMember: async (payload: { user_id: number; department_id: number; role: string }): Promise<DepartmentMember> => {
    const response = await fetch(`${API_BASE}/api/org/department-members`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify(payload),
    });
  ...
  updateDepartmentMember: async (memberId: number, payload: { role?: string }): Promise<DepartmentMember> => {
    const response = await fetch(`${API_BASE}/api/org/department-members/${memberId}`, {
      method: 'PUT',
      headers: getHeaders(),
      body: JSON.stringify(payload),
    });
  ...
  listOrgProjects: async (): Promise<Project[]> => {
    const response = await fetch(`${API_BASE}/api/org/projects`, { headers: getHeaders() });
    return handleResponse(response);
  },

  // 通过名称获取组织架构项目（用于与项目空间关联）
  getOrgProjectByName: async (name: string): Promise<Project | null> => {
    const response = await fetch(`${API_BASE}/api/org/projects/by-name/${encodeURIComponent(name)}`, { headers: getHeaders() });
    if (!response.ok) {
      return null;
    }
    return handleResponse(response);
  ...
  createProject: async (payload: { name: string; description?: string; is_public: boolean; department_ids?: number[] }): Promise<Project> => {
    ...
    try {
      ...
      const orgResponse = await fetch(`${API_BASE}/api/org/projects`, {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({
          name: payload.name,
  ...
  getProject: async (projectId: number): Promise<Project> => {
    const response = await fetch(`${API_BASE}/api/org/projects/${projectId}`, { headers: getHeaders() });
    return handleResponse(response);
  },
  ...
  updateProject: async (projectId: number | string, payload: { name?: string; description?: string; is_public?: boolean }, orgProjectId?: number): Promise<Project> => {
    ...
    if (orgProjectId !== undefined && orgProjectId !== null) {
      try {
        // 2. 更新组织架构系统的项目信息
        const response = await fetch(`${API_BASE}/api/org/projects/${orgProjectId}`, {
          method: 'PUT',
          headers: getHeaders(),
          body: JSON.stringify(payload),
        });
