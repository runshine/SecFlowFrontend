...
/** R3 合并：W+J 必须同时 passed；W=skip(无外部输入) 整体跳过 */
function combineR3(r3w: FuncStage, r3j: FuncStage): FuncStage {
  if (r3w === 'skip') return 'skip';
  if (r3w === 'passed' && r3j === 'passed') return 'passed';
  if (r3w === 'running' || r3j === 'running') return 'running';
  if (r3w === 'failed'  || r3j === 'failed')  return 'running'; // 重试中
  if (r3w === 'passed')  return 'running';   // W 完成，J 即将启动
  return 'pending';
}
 ...
 *   R5:   r5_state    → catalog.rep_state
 *
 * 核心规则：
 *   R3 = combineR3(r3w, r3j)：W+J 全部 passed 才算 R3 通过
 *   R4 = 以 r4_state 为权威。r4_decision 由 R3-W 提前写入，在 r4_state=passed
 *        前不能作为 R4 完成信号。单文件模块 engine 已在绕过 R4 时设 r4_state=PASSED。
 *   r4_decision=filter + has_input=false → R4 无需运行，显示 skip
 *   r4_decision=filter + has_input=true  → R3/R4 决定过滤，显示 remove
 */
function deriveFuncProgress(
  ...
  const TERMINAL = new Set<FuncStage>(['passed', 'failed', 'keep', 'remove', 'skip']);
  const isT = (s: FuncStage) => TERMINAL.has(s);
  ...
  const getOrCreate = (fh: string, name?: string, file?: string): FuncProgress => {
    if (!map.has(fh)) {
      map.set(fh, {
        ...
        r2j: 'pending', r3w: 'pending', r3j: 'pending', r3: 'pending',
  ...
  /** 仅当目标字段非终态时推进 */
  const adv = (f: FuncProgress, field: keyof Pick<FuncProgress,'r2j'|'r3w'|'r3j'|'r3'|'r4'|'rep'>, next: FuncStage) => {
    if (!isT(f[field])) f[field] = next;
  };
  ...
  for (const item of functionCatalog || []) {
    ...
    const hi       = item.has_external_input == null ? undefined : Boolean(item.has_external_input);
    if (r4dec === 'filter' || r4dec === 'remove') {
      // R3 或 R4 决定过滤，R4 不需要运行（或已运行并过滤）
      f.r4 = (hi === false) ? 'skip' : 'remove';
    } else if (r4dec === 'keep') {
      // R3 批准，只有 r4_state=passed 才算 R4 真正完成
  ...
  // ── Step 2: Events 叠加实时「运行中」状态 ────────────────────────────────
  // 原则：只把非终态推进到 running；不覆盖 catalog 已设的终态
  for (const evt of events) {
    const ts = evt.ts || 0;
    const d  = evt.data || {};
    ...
    switch (evt.type) {
      // R2-J
      case 'r2_j_start':
        adv(f, 'r2j', 'running'); f.lastTs = ts; break;
      case 'r2_j_done':
        adv(f, 'r2j', d.passed ? 'passed' : 'failed');
        if (!d.passed) { adv(f,'r3','skip'); adv(f,'r4','skip'); adv(f,'rep','skip'); }
        f.lastTs = ts; break;

      // R3-W
      case 'r3_w_start':
        adv(f, 'r3w', 'running'); adv(f, 'r3', 'running'); f.lastTs = ts; break;
      case 'r3_w_done': {
        const hi2 = Boolean(d.has_external_input);
        adv(f, 'r3w', hi2 ? 'passed' : 'skip');
        f.has_external_input = hi2;
        if (d.entry_role) f.entry_role = String(d.entry_role);
        if (!hi2) { adv(f,'r3j','skip'); adv(f,'r3','skip'); adv(f,'r4','skip'); }
        else       adv(f,'r3', 'running');
        f.lastTs = ts; break;
      }

      // R3-J
      case 'r3_j_start':
        adv(f, 'r3j', 'running'); adv(f, 'r3', 'running'); f.lastTs = ts; break;
      case 'r3_j_done':
        // 失败时重置为 pending（重试）；Step3 会重算 r3
        adv(f, 'r3j', d.passed ? 'passed' : 'pending');
        f.lastTs = ts; break;

      // R4-W（engine 发出 r4_w_start/done）
      case 'r4_w_start':
        adv(f, 'r4', 'running'); f.lastTs = ts; break;
      case 'r4_w_done': {
        // r4_w_done 时 r4_state 可能已由 engine 设 passed；先用 decision 判断过滤
        const dec = String(d.decision || '').toLowerCase();
        if (dec === 'filter' || dec === 'remove') {
          adv(f, 'r4', f.has_external_input === false ? 'skip' : 'remove');
        }
        // keep: 需等 r4_j_done 确认（或 catalog 刷新后 r4_state=passed 才显示 keep）
        f.lastTs = ts; break;
      }

      // R4-J（engine 发出 r4_j_start/done）
      case 'r4_j_start':
        adv(f, 'r4', 'running'); f.lastTs = ts; break;
      case 'r4_j_done':
        if (d.passed) { adv(f,'r4','keep'); f.is_entry = true; adv(f,'rep','pending'); }
        f.lastTs = ts; break;

      // R5
      case 'r5_w_start':
        adv(f, 'rep', 'running'); f.lastTs = ts; break;
      case 'r5_j_done':
        adv(f, 'rep', d.passed ? 'passed' : 'failed'); f.lastTs = ts; break;

      default: break;
    }
  }

  // ── Step 3: 统一重算 r3 + is_entry ──────────────────────────────────────
  for (const f of map.values()) {
    // r3 以 catalog 终态为准，非终态才用 combineR3 计算
    if (!isT(f.r3)) f.r3 = combineR3(f.r3w, f.r3j);
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-500">
                    {[
                      ...
                      <span key={label} className="inline-flex items-center gap-1.5">
                        ...
                        <span className="inline-block w-20 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                          <span className={`block h-full rounded-full ${color} transition-all`}
                ...
                <table className="w-full min-w-[680px] text-xs">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
                      ...
                      <th className="px-2 py-2.5 text-center" title="R4 入口决策 W+J（以 r4_state 为权威）">R4</th>
